...
module.exports = {
  ...
  'datapoint-history': async (request, h) => {
    let limit = 100;
    let offset = 0;
  ...
  'datapoint-latest-json': async (request, h) => {
    const { influx } = request.server.plugins.influx;
  ...
  'datapoint-latest-txt': async (request, h) => {
    const { influx } = request.server.plugins.influx;
  ...
  'datapoint-feed': async (request, h) => h.response('RSS FEED PEW PEW PEW'),

  // Update value
  // /s/vhs/data/isopen/update?value=closed
  'datapoint-update-put': async (request, h) => {
    // Check input
  ...
  //
  'datapoint-update-get': async (request, h) => {
  ...
  'datapoint-fullpage': async (request, h) => {
    const { influx } = request.server.plugins.influx;
  ...
  'combined-fullpage': async (request, h) => {
    const { influx } = request.server.plugins.influx;
  ...
  'datapoint-index': async (request, h) => {
    const { influx } = request.server.plugins.influx;
